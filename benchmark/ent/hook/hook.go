...
// The OrderNoteFunc type is an adapter to allow the use of ordinary
// function as OrderNote mutator.
type OrderNoteFunc func(context.Context, *ent.OrderNoteMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f OrderNoteFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.OrderNoteMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.OrderNoteMutation", m)
}
