...
func (_q *OrderQuery) QueryNotes() *OrderNoteQuery {
	query := (&OrderNoteClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *OrderQuery) WithNotes(opts ...func(*OrderNoteQuery)) *OrderQuery {
	query := (&OrderNoteClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *OrderQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Order, error) {
	...
	if query := _q.withNotes; query != nil {
		if err := _q.loadNotes(ctx, query, nodes,
			func(n *Order) { n.Edges.Notes = []*OrderNote{} },
			func(n *Order, e *OrderNote) { n.Edges.Notes = append(n.Edges.Notes, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *OrderQuery) loadNotes(ctx context.Context, query *OrderNoteQuery, nodes []*Order, init func(*Order), assign func(*Order, *OrderNote)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Order)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(ordernote.FieldOrderID)
	}
	query.Where(predicate.OrderNote(func(s *sql.Selector) {
