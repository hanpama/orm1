...
import (
	...
	"github.com/hanpama/orm1/benchmark/ent/migrate"
...
type Client struct {
	...
	// OrderNote is the client for interacting with the OrderNote builders.
	OrderNote *OrderNoteClient
...
func (c *Client) init() {
	...
	c.OrderNote = NewOrderNoteClient(c.config)
...
func (c *Client) Tx(ctx context.Context) (*Tx, error) {
	...
	return &Tx{
		...
		OrderNote: NewOrderNoteClient(cfg),
...
func (c *Client) Intercept(interceptors ...Interceptor) {
	...
	c.OrderNote.Intercept(interceptors...)
...
func (c *Client) Mutate(ctx context.Context, m Mutation) (Value, error) {
	...
	case *OrderNoteMutation:
		return c.OrderNote.mutate(ctx, m)
	case *UserMutation:
...
func (c *OrderClient) QueryNotes(_m *Order) *OrderNoteQuery {
	query := (&OrderNoteClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := _m.ID
		step := sqlgraph.NewStep(
			sqlgraph.From(order.Table, order.FieldID, id),
			sqlgraph.To(ordernote.Table, ordernote.FieldID),
...
// OrderNoteClient is a client for the OrderNote schema.
type OrderNoteClient struct {
	config
}

// NewOrderNoteClient returns a client for the OrderNote from the given config.
func NewOrderNoteClient(c config) *OrderNoteClient {
	return &OrderNoteClient{config: c}
}

// Use adds a list of mutation hooks to the hooks stack.
// A call to `Use(f, g, h)` equals to `ordernote.Hooks(f(g(h())))`.
func (c *OrderNoteClient) Use(hooks ...Hook) {
	c.hooks.OrderNote = append(c.hooks.OrderNote, hooks...)
}

// Intercept adds a list of query interceptors to the interceptors stack.
// A call to `Intercept(f, g, h)` equals to `ordernote.Intercept(f(g(h())))`.
func (c *OrderNoteClient) Intercept(interceptors ...Interceptor) {
	c.inters.OrderNote = append(c.inters.OrderNote, interceptors...)
}

// Create returns a builder for creating a OrderNote entity.
func (c *OrderNoteClient) Create() *OrderNoteCreate {
	mutation := newOrderNoteMutation(c.config, OpCreate)
	return &OrderNoteCreate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// CreateBulk returns a builder for creating a bulk of OrderNote entities.
func (c *OrderNoteClient) CreateBulk(builders ...*OrderNoteCreate) *OrderNoteCreateBulk {
	return &OrderNoteCreateBulk{config: c.config, builders: builders}
}

// MapCreateBulk creates a bulk creation builder from the given slice. For each item in the slice, the function creates
// a builder and applies setFunc on it.
func (c *OrderNoteClient) MapCreateBulk(slice any, setFunc func(*OrderNoteCreate, int)) *OrderNoteCreateBulk {
	rv := reflect.ValueOf(slice)
	if rv.Kind() != reflect.Slice {
		return &OrderNoteCreateBulk{err: fmt.Errorf("calling to OrderNoteClient.MapCreateBulk with wrong type %T, need slice", slice)}
	}
	builders := make([]*OrderNoteCreate, rv.Len())
	for i := 0; i < rv.Len(); i++ {
		builders[i] = c.Create()
		setFunc(builders[i], i)
	}
...
// Update returns an update builder for OrderNote.
func (c *OrderNoteClient) Update() *OrderNoteUpdate {
	mutation := newOrderNoteMutation(c.config, OpUpdate)
	return &OrderNoteUpdate{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOne returns an update builder for the given entity.
func (c *OrderNoteClient) UpdateOne(_m *OrderNote) *OrderNoteUpdateOne {
	mutation := newOrderNoteMutation(c.config, OpUpdateOne, withOrderNote(_m))
	return &OrderNoteUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOneID returns an update builder for the given id.
func (c *OrderNoteClient) UpdateOneID(id int64) *OrderNoteUpdateOne {
	mutation := newOrderNoteMutation(c.config, OpUpdateOne, withOrderNoteID(id))
	return &OrderNoteUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// Delete returns a delete builder for OrderNote.
func (c *OrderNoteClient) Delete() *OrderNoteDelete {
	mutation := newOrderNoteMutation(c.config, OpDelete)
	return &OrderNoteDelete{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// DeleteOne returns a builder for deleting the given entity.
func (c *OrderNoteClient) DeleteOne(_m *OrderNote) *OrderNoteDeleteOne {
	return c.DeleteOneID(_m.ID)
}

// DeleteOneID returns a builder for deleting the given entity by its id.
func (c *OrderNoteClient) DeleteOneID(id int64) *OrderNoteDeleteOne {
	builder := c.Delete().Where(ordernote.ID(id))
	builder.mutation.id = &id
	builder.mutation.op = OpDeleteOne
	return &OrderNoteDeleteOne{builder}
}

// Query returns a query builder for OrderNote.
func (c *OrderNoteClient) Query() *OrderNoteQuery {
	return &OrderNoteQuery{
		config: c.config,
		ctx:    &QueryContext{Type: TypeOrderNote},
		inters: c.Interceptors(),
	}
}

// Get returns a OrderNote entity by its id.
func (c *OrderNoteClient) Get(ctx context.Context, id int64) (*OrderNote, error) {
	return c.Query().Where(ordernote.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *OrderNoteClient) GetX(ctx context.Context, id int64) *OrderNote {
	obj, err := c.Get(ctx, id)
	if err != nil {
		panic(err)
	}
...
// QueryOrder queries the order edge of a OrderNote.
func (c *OrderNoteClient) QueryOrder(_m *OrderNote) *OrderQuery {
	query := (&OrderClient{config: c.config}).Query()
	query.path = func(context.Context) (fromV *sql.Selector, _ error) {
		id := _m.ID
		step := sqlgraph.NewStep(
			sqlgraph.From(ordernote.Table, ordernote.FieldID, id),
			sqlgraph.To(order.Table, order.FieldID),
...
// Hooks returns the client hooks.
func (c *OrderNoteClient) Hooks() []Hook {
	return c.hooks.OrderNote
}

// Interceptors returns the client interceptors.
func (c *OrderNoteClient) Interceptors() []Interceptor {
	return c.inters.OrderNote
}

func (c *OrderNoteClient) mutate(ctx context.Context, m *OrderNoteMutation) (Value, error) {
	switch m.Op() {
	case OpCreate:
		return (&OrderNoteCreate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdate:
		return (&OrderNoteUpdate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdateOne:
		return (&OrderNoteUpdateOne{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpDelete, OpDeleteOne:
		return (&OrderNoteDelete{config: c.config, hooks: c.Hooks(), mutation: m}).Exec(ctx)
	default:
		return nil, fmt.Errorf("ent: unknown OrderNote mutation op: %q", m.Op())
	}
}
...
type (
	hooks struct {
		Order, OrderItem, OrderNote, User []ent.Hook
	}
	inters struct {
		Order, OrderItem, OrderNote, User []ent.Interceptor
	}
)
